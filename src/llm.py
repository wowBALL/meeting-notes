...
# gemma4 พังเฉพาะตอนต้องผ่าน reduce (ดูคอมเมนต์ที่ GEMMA_MAP_MAX_TOKENS ด้านบน) แต่
# single-call (ข้าม chunk ไปเลย) วัดจริงหลายระดับแล้วผ่านหมด ไม่เจอ error หรือเนื้อหา
# หายเลยสักครั้ง (วัด 2026-08-05):
#   - ประชุมจริง 84 นาที (~36,210 token): 33.1s, ครอบคลุมทั้งประชุม
#   - ประชุมจริงที่ยาวที่สุดที่มี (~150 นาที, ~92,650 token): 58.5s, สรุปอ้างถึงช่วง
#     [144:07]-[150:00] ได้ -- แปลว่าอ่านจบจริง ไม่ใช่แค่ต้นประชุม
#   - ทดสอบสังเคราะห์ (ต่อ transcript หลายไฟล์เข้าด้วยกัน ไม่ต้องมีเนื้อหาต่อเนื่องกันจริง
#     แค่ต้องการดัน token ให้เกินกว่าที่มีในไฟล์จริง) ไล่ระดับ 137K → 181K → 222K →
#     259,729 token -- **ผ่านหมดทุกระดับ ไม่เจอ error เลย** เพดานบนที่วัดได้จริง
#     (259,729) ใกล้ 256K (262,144) ที่ Google ประกาศเป็น context window ของ Gemma 4
#     รุ่นกลาง (12B/26B-A4B/31B) มาก แปลว่า endpoint นี้น่าจะรันรุ่นกลางตัวใดตัวหนึ่ง
#     (source: https://ai.google.dev/gemma/docs/core)
#
# ไม่ตั้งเพดานที่ตัวเลขที่วัดได้ตรงๆ (259,729) เพราะ estimate_tokens() เป็นค่าประมาณ
# ไม่ใช่ tokenizer จริงของโมเดล การอยู่ชิดขอบสถาปัตยกรรมขนาดนี้เสี่ยงเกินไป และประชุมจริง
# ที่ยาวที่สุดที่มีอยู่ก็แค่ ~92,650 token -- 150,000 คือเลขที่เผื่อพอสำหรับประชุมจริงทุก
# แบบที่มี (มากกว่าตัวที่ยาวสุด ~62%) แต่ยังเหลือระยะห่างจากขอบ 256K อีกเยอะ ถ้าเจอประชุม
# ที่ยาวกว่านี้จริงแล้วอยากรู้ผลจริงต้องวัดใหม่ ไม่ใช่ขยับเพดานเอาเอง
GEMMA_SINGLE_CALL_THRESHOLD_TOKENS = 150_000
