...
# ถือว่าไม่รู้จัก
#
# วัดจริง 2026-07-30 ด้วยท่อทั้งเส้นของ src/voiceprint.py บน Meet1900 เต็มไฟล์ (81 นาที,
# community-1, 2143 turn, 6 label) บวกคลิป enroll 20 วินาที -- วัดซ้ำได้ด้วย
# `python -m tools.calibrate_voiceprints speakers-truth.json --cache .calib-cache`:
#
#   คนเดียวกัน ข้ามการบันทึก (คลิป enroll เทียบ SPEAKER_05)   0.7493   <- ตัวที่สำคัญที่สุด
#   คนละคน ข้ามการบันทึก สูงสุด (จาก 5 คู่)                    0.4310
#   คนละคน ในไฟล์เดียวกัน สูงสุด (จาก 15 คู่)                  0.6313
#   คนเดียวกัน ในไฟล์เดียวกัน (split-half, 7 คู่)              0.8042 - 0.9195
#
# HIGH = 0.70 อยู่กลางหน้าต่าง (0.6313, 0.7493] ห่างสองฝั่งราว 0.05 -- ทั้งสองฝั่งเป็นความ
# เสียหายจริงแต่ไม่เท่ากัน: สูงเกินไป = ไม่ใส่ชื่อคนที่จำได้ (คลิกแก้ได้) ต่ำเกินไป = ใส่ชื่อ
# ผิดคนลงสรุปที่ระบุผู้รับผิดชอบ (แก้ไม่ได้ถ้าไม่มีใครสังเกต) ที่เลือกกลางแทนที่จะเอียงขึ้น
# เพราะขอบล่างของหน้าต่างมาจากคู่เดียว ยังไม่รู้ว่ามันเป็นคู่คนละคนจริงหรือเป็นคนเดียวที่
# pyannote แยกเป็นสองป้าย -- เอียงขึ้นตอนนี้คือเชื่อจุดข้อมูลที่ยังไม่ได้ยืนยัน
#
# LOW = 0.45 อยู่เหนือเพดานของคู่คนละคนข้ามการบันทึก (0.4310) เล็กน้อย
#
# *** ทำไมไม่ใช่ 0.80 เหมือนก่อนหน้านี้ ***: 0.80 วัดมาจากพื้นที่ของ centroid ที่ pyannote
# คำนวณเพื่อ clustering ซึ่งไม่ใช่พื้นที่นี้ ในพื้นที่ใหม่ 0.80 จะปฏิเสธคู่ที่ถูกต้องที่สุด
# ที่วัดได้ (0.7493) ทั้งที่มันห่างจากอันดับสองถึง 0.32 -- ตัวเลขเก่าที่ยกมาใช้ต่อในพื้นที่
# ใหม่คือค่าที่ดูสมเหตุสมผลแต่ผิดอย่างเงียบที่สุด
#
# พื้นที่ใหม่แยกคนได้ดีกว่าเดิมจริงในจุดที่สำคัญ: พื้นที่เก่ามีคู่คนละคนชนกันถึง 0.741 และ
# 0.691 (เหตุผลที่ต้องดัน HIGH ขึ้นไป 0.80) พื้นที่ใหม่คู่คนละคนข้ามการบันทึกสูงสุด 0.4310
# ส่วนคะแนนคนเดียวกันข้ามการบันทึกได้ 0.7493 เทียบกับ 0.757 ของพื้นที่เก่า คือเท่าเดิม --
# สิ่งที่ดีขึ้นคือเพดานของคนผิด ไม่ใช่คะแนนของคนถูก
#
...
DEFAULT_SPEAKER_MATCH_LOW = 0.45
