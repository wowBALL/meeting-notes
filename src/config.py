...
#
#   คลิป enroll เทียบป้ายที่ใกล้ที่สุดในประชุม (SPEAKER_03)   0.6679
#   คลิป enroll เทียบป้ายที่เหลือ สูงสุด (จาก 5 คู่)           0.4288
#   คนละคน ในประชุมเดียวกัน สูงสุด (จาก 15 คู่)               0.5711
#
# *** ตัวตนของ SPEAKER_03 ยังไม่ได้ยืนยันด้วยหู *** เจ้าของเสียงฟังคลิปแล้วจำไม่ได้ว่า
# ป้ายไหนคือตัวเอง (2026-07-31) จึงยัง "ไม่มี" คู่คนเดียวกันข้ามการบันทึกที่ยืนยันแล้วแม้แต่
# คู่เดียวในชุดข้อมูลนี้ -- 0.6679 เป็นแค่ตัวที่โดดออกมา ไม่ใช่คู่ที่พิสูจน์แล้ว การเอาคะแนน
# มาสรุปเองว่า "คะแนนสูงสุดต้องคือเขา" แล้วตั้งเกณฑ์จากข้อสรุปนั้น คือการเอาคำตอบมาพิสูจน์
# ตัวมันเอง
#
# HIGH = 0.70 จึงเลือกให้ "ปลอดภัยไม่ว่าคำตอบจะเป็นทางไหน" ไม่ใช่กลางช่องว่าง:
#   ถ้า SPEAKER_03 คือเขาจริง -> 0.6679 ตกไปอยู่โซน "เสนอให้ยืนยัน" เสียหนึ่งคลิก และคะแนน
#     ถูกบันทึกไว้ให้เอามาวัดต่อ
#   ถ้าไม่ใช่ -> 0.6679 คือคู่คนละคน และ 0.70 กันมันไม่ให้ใส่ชื่อผิดได้พอดี
# ไม่มีคู่ไหนในชุดข้อมูลนี้แตะ 0.70 เลย แปลว่าตอนนี้ระบบจะไม่ใส่ชื่อใครอัตโนมัติจนกว่าจะมี
# หลักฐานที่ดีกว่านี้ -- ซึ่งเป็นสิ่งที่ถูกต้องสำหรับเกณฑ์ที่ยังไม่มีคู่ยืนยันสักคู่
#
# LOW = 0.45 อยู่เหนือ 0.4288 (ป้ายที่เหลือทั้งหมดของประชุม เทียบกับคลิป enroll -- คู่ที่
# แน่ใจได้ว่าคนละคนอย่างน้อยสี่ในห้าคู่) เล็กน้อย
#
# *** ทำไมไม่ใช่ 0.80 เหมือนก่อนหน้านี้ ***: 0.80 วัดมาจากพื้นที่ของ centroid ที่ pyannote
# คำนวณเพื่อ clustering ซึ่งไม่ใช่พื้นที่นี้ ในพื้นที่ใหม่ไม่มีคู่ไหนแตะ 0.60 ด้วยซ้ำ 0.80
