...
| `COMPANION_COMMAND` | *(ไม่ตั้ง)* | รันโปรแกรมหนึ่งตัวคู่ไปกับการอัด แล้วปิดให้อัตโนมัติ — ดูหัวข้อถัดไป |

#### `COMPANION_COMMAND` — รันโปรแกรมอื่นคู่ไปกับการอัด (ไม่บังคับ)

ไม่ตั้งค่า = ไม่มีอะไรเกิดขึ้น ซึ่งเป็นค่าเริ่มต้น

เมื่อตั้งไว้ โปรแกรมจะถูกเปิดตอนเริ่มอัดและถูกปิดตอนการอัดเข้าสู่**ขั้นแปลงไฟล์** (ไม่ใช่ตอนแปลงเสร็จ) — จังหวะนี้เลือกไว้เพื่อให้ทรัพยากรระดับเครื่องที่โปรแกรมนั้นถืออยู่ เช่นหน่วยความจำการ์ดจอ ถูกคืนทันก่อนขั้นถอดเสียงจะขอใช้ วัดจากประชุมจริงพบว่าช่วงแปลงไฟล์กว้าง 62–69 วินาที ส่วนช่วงหลังแปลงเสร็จถึงงานถัดไปมีแค่ราว 2 วินาที
...
โปรแกรมลูกได้รับตัวแปร `MEETING_ROOM` เป็นชื่อห้องที่กำลังอัด (สตริงว่างถ้าไม่ได้ตั้งชื่อ) เพิ่มจากตัวแปรสภาพแวดล้อมทั้งหมดของ service

ข้อจำกัดที่ตั้งใจ: โปรแกรมนี้ถือเป็น**ของเสริม** ถ้าเปิดไม่ติดหรือปิดไม่ลง การประชุมจะเดินต่อตามปกติและมีแค่บรรทัดใน log — จะไม่มีวันทำให้เปิดห้องไม่ได้ และถ้ามันไม่ยอมปิดเองภายใน 5 วินาทีจะถูกบังคับปิด ค่าที่ตั้งผิดรูป (ไม่ใช่ JSON, ไม่ใช่ list, มีสมาชิกที่ไม่ใช่สตริง) ก็แปลว่าปิดฟีเจอร์ ไม่ใช่เปิดห้องไม่ได้
