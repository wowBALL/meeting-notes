...
1. **เมนูเลือกโมเดลสรุป** — `[1] GLM 5.2` ข้อมูลไม่ออกนอกบริษัทแต่ช้ากว่า / `[2] Opus 5` แม่นกว่า / `[3] Sonnet 5` ถูกกว่า / `[4] ถอดเสียงอย่างเดียว` ไม่ส่งให้โมเดลไหนเลย / `[5] Qwen 3.6` ข้อมูลไม่ออกนอกบริษัทเหมือน GLM แต่เร็วกว่ามาก (กด Enter = GLM 5.2)
...
├── summary.md          สรุปแยก "ตกลงแล้ว" ออกจาก "เสนอไว้" + action items (เนื้อหาล้วน ส่งต่อให้คนอ่านได้ตรงๆ — ไม่มีไฟล์นี้ถ้าเลือก [4])
├── summary.meta.md     โมเดลที่ใช้สรุป, ประเภทประชุม, คำที่แก้ตาม glossary -- ข้อมูลของระบบ ไม่ใช่เนื้อหาประชุม
...
**ถ้า GLM ล่ม ระบบจะไม่ไปเรียก Claude แทนให้** ประชุมนั้นจะได้แค่ transcript การสลับไปหา provider อื่นเองเท่ากับส่งบทสนทนาออกนอกบริษัทโดยที่ไม่มีใครสั่ง ซึ่งเป็นเหตุผลตั้งต้นที่มีตัวเลือกนี้อยู่ — `[5] Qwen 3.6` มีไว้สำหรับกรณีนี้: มันอยู่บน endpoint เดียวกัน ข้อมูลจึงไม่ออกนอกบริษัทเหมือนกัน แต่ต้องเป็นคนเลือกเอง ระบบไม่สลับให้เอง
