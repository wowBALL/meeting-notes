...
def test_the_default_summary_model_is_the_one_that_keeps_data_in_house():
    ...
    ในสามตัวที่ข้อมูลไม่ออกนอกบริษัท ตรึงไว้ที่ Qwen ไม่ใช่ GLM-5.2 หรือ gemma4: GLM คืน
    content ว่างเปล่ากับงานสรุป 10 ครั้งจาก 10 ครั้ง (วัด 2026-07-31) ส่วน gemma4 ถูกลองเป็น
    ค่าเริ่มต้นเมื่อ 2026-08-05 แล้วถอยกลับวันเดียวกันหลังพบว่ามันยุบขั้น reduce เหลือสรุปแค่
    chunk แรกของประชุมจริงแบบเงียบๆ ไม่มี TRUNCATION_NOTICE (ดูคอมเมนต์ที่ตัวแปรนี้ใน
    src/config.py) ส่วน Qwen เสร็จใน 4 วินาทีและครบทุก chunk
    """
